...
def get_dedup_list(param):
    query_text = "| readFile format=parquet path=kw_ne_ind | fields %s | dedup %s" % (param, param)
    cache_ttl = 59
    tws = 11
    twf = 22
...
def get_filtered_archive(ne, kw):
    query_text = "| readFile format=parquet path=kw_ne_ind "
...
def get_article_by_index(index):
    query_text = "| readFile format=parquet path=text_ind "
...
def get_unique_source_topics():
    query_text = "| readFile format=parquet path=kw_ne_ind | eval group = source+\":\"+topic | dedup group "
    cache_ttl = 59
    tws = 11
    twf = 22
    job = conn.jobs.create(query_text=query_text, cache_ttl=cache_ttl, tws=tws, twf=twf, blocking=True)
